...
class UserControllerTest {
    ...
    public void find_user_by_id_not_found_test() {
        ...
        Assertions.assertEquals("The user " +  userId + " doesn't exist in the data base", response.getBody());
    }
    ...
    public void create_user_success_test() {
        // Arrange
        UserDto userDto = new UserDto("John", "Doe", LocalDate.of(1989, 5, 10), "john@example.com", "password");
        UserResponseDto userResponseDto = new UserResponseDto("123", "John", "Doe", LocalDate.of(1989, 5, 10), "john@example.com");
        Mockito.when(userService.createUser(userDto)).thenReturn(userResponseDto);

        // Act
        ResponseEntity<UserResponseDto> response = userController.createUserAdmin(userDto);
